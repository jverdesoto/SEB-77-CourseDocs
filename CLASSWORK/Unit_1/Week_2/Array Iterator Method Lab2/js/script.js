...
const firstLastName = inventors.map(inventor => ({ first: inventor.first, last: inventor.last }));
...
const adaFirstName = inventors.find(inventor => inventor.first === 'Ada')
console.log(adaFirstName)
...
const data = [
  ...
  const instanceNum = data.reduce((acc, dat) => {
...
const areAllNineteenOrOlder = devs.every(dev => (currentYear - dev.year) >= 19);
...
console.log(areAllNineteenOrOlder);
  ...
  const indexToFind = comments.findIndex(comment => comment.id === 123523);
