...
function findYearOfAlbum(nameOfAlbum) {
    let year
    pinkFloyd.discography.map(album => {
      if (album.name === nameOfAlbum) 
        year = album.year
    })
    return year
}
...
function findBandMember(name) {
    let role
    pinkFloyd.members.map(memberProfile => {
        if (memberProfile.name === name) 
            role = memberProfile.role
    })
    return role
}

console.log(findBandMember('David Gilmour'));