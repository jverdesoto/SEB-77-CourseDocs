...
function formatProducts(carousel, grid, sidebar) {
  ...
  const carouselProductsReformatted = carousel.map(replaceSpacesCapitalize);
  const gridProductsReformatted = grid.map(replaceSpacesCapitalize);
  const sidebarProductsReformatted = sidebar.map(replaceSpacesCapitalize);

  function replaceSpacesCapitalize (product) {
