...
function formatProducts(carousel, grid, sidebar) {
  const carouselProductsReformatted = carouselProducts.map(formatName)

  const gridProductsReformatted = gridProducts.map(formatName)

  const sidebarProductsReformatted = sidebarProducts.map(formatName)
