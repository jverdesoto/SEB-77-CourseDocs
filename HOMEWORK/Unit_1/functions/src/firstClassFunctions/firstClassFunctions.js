...
function formatProducts(carousel, grid, sidebar) {
  const allShops = [carousel, grid, sidebar]
  const carouselProductsReformatted = allShops.map((shop) => {

    const allProducts = shop.map((product) => {
  ...
  return carouselProductsReformatted
}
